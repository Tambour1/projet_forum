        ...
        <div v-for="(group, groupIndex) in items" :key="groupIndex">
            ...
            <div v-show="openGroup === groupIndex">
                <div v-for="(item, itemIndex) in forumsStore.getForums.slice(0, visibleCounts[groupIndex])"
                    ...
                    <button @click="toggleItem(itemIndex)"
                        class="w-full flex justify-between items-start px-3 py-2 hover:bg-[#2a2a2b] rounded text-sm text-gray-200 hover:text-gray-300">
                        <span class="break-words whitespace-normal w-[85%] text-left">
                            {{ item.name }}
                        </span>
